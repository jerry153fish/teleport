...
func TestShutdown(t *testing.T) {
	...
	defer ctxc()
	require.Equal(t, trace.IsConnectionProblem(srv.Shutdown(ctx)), true)
...
func TestConfigureCiphers(t *testing.T) {
	...
	require.NoError(t, err, fmt.Sprintf("cipher match, should not fail, got error: %v", err))
	defer clt.Close()
}
...
func TestHostSignerFIPS(t *testing.T) {
	...
	for _, tt := range tests {
		...
		require.Equal(t, err != nil, tt.outError)
	}
}

func wait(t *testing.T, srv *Server) {
	s := make(chan struct{})
	go func() {
	...
	case <-time.After(time.Second):
		require.Equal(t, false, true, "exceeded waiting timeout")
	case <-s:
	}
}

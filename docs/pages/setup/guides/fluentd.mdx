...
<Tabs>
  ...
  ```
  <Notice type="warning">
  To run the event handler plugin on a machine with an M1 chip, you will need to
  ...
  Run the following commands to build the plugin and move it to the root of the
  `event-handler` directory:

  ```code
  $ git clone https://github.com/gravitational/teleport-plugins.git --depth 1
  $ cd teleport-plugins/event-handler/build.assets
  $ make install
  $ cd ../../.. && mv teleport-plugins/event-handler/build/teleport-event-handler .
  ```
  </TabItem>
